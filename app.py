...
啟動方式:
    cd MI_Prediction/src_pipeline_viz
...
# ── 讓 app.py 能 import MI_Prediction 的模組 ────────────────────────
_MI_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
...
# decode_label 定義在 api/label_decoder.py（不含 FastAPI 依賴）
sys.path.insert(0, os.path.join(_MI_DIR, "api"))
...
# ── Bull's-Eye 圖片目錄（3D_2D plot 專案的預渲染 PNG）────────────────
_BULLSEYE_DIR = os.path.abspath(
    os.path.join(
        os.path.dirname(__file__),
        "../../3D_2D plot/GUI_12LeadECG/templates/static/images/area_2d_bullseye_vessel",
    )
)

# ── MI 深度學習模型路徑（依序嘗試：相對路徑 → Docker 絕對路徑 → 環境變數）──
_MI_MODEL_DIR_CANDIDATES = [
    os.path.abspath(os.path.join(os.path.dirname(__file__), "../../MI_model_20260416")),
    "/app/MI_model_20260416",          # Docker 容器絕對路徑
    os.environ.get("MI_MODEL_DIR", ""), # 環境變數備援
]
_MI_MODEL_DIR = next(
    (p for p in _MI_MODEL_DIR_CANDIDATES if p and os.path.isdir(p)),
    _MI_MODEL_DIR_CANDIDATES[0]
)
if _MI_MODEL_DIR not in sys.path:
    sys.path.insert(0, _MI_MODEL_DIR)
...
# ── 常數 ─────────────────────────────────────────────────────────────
TRAINSET_PATH = os.path.join(_MI_DIR, "src_trainset.npz")
...
_IMG_DIR  = os.path.join(_VIZ_DIR, "亮色風格 阻塞情境")
