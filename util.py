...
def my_eval_with_dynamic_thresh(gt, pred, input_thresholds=None):
    """
    Evaluates the model with dynamically adjusted thresholds for each task.
    ...
    """
# 判斷要自動尋找門檻還是使用傳入的
    if input_thresholds is not None:
        optimal_thresholds = input_thresholds
        # 確保格式是 list 或 array 以對應多任務
        if isinstance(optimal_thresholds, (float, int)):
            optimal_thresholds = [float(optimal_thresholds)]
    else:
        optimal_thresholds = find_optimal_thresholds(gt, pred)
        
    n_task = gt.shape[1]
    rocaucs, sensitivities, specificities, f1, auprcs = [], [], [], [], []
